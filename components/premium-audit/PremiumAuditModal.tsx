...
import { createCheckoutSession, getStripePublicKey } from "@/app/actions/checkout";
...
const AUDIT_GOALS = [
  "Market Fit — is this worth building?",
  "UI/UX Design — is it easy to use?",
  "Tech Foundation — is it built to last?",
  "Go-To-Market Strategy — how do I reach customers?",
  "Revenue Model — can this make money?",
...
const premiumAuditSchema = z
  .object({
    ...
    auditGoals: z
      .array(z.string())
      .min(1, "Select at least one audit focus area."),
    ...
    payingCustomers: z.string().min(1, "Paying customer status is required."),
...
type PremiumAuditFormValues = z.infer<typeof premiumAuditSchema>;
type Step = 1 | 2 | 3 | 4;

const DEFAULT_VALUES: PremiumAuditFormValues = {
  agreedToTerms: false,
  auditGoals: [],
  builtWith: [],
  email: "",
...
const STEP_FIELDS: Record<Step, Array<keyof PremiumAuditFormValues>> = {
  1: ["productName", "projectStage", "problem", "prototypeLink"],
  2: ["launchStatus", "payingCustomers", "revenueModel", "auditGoals"],
  3: ["name", "email", "phone", "agreedToTerms"],
  4: [],
};
      ...
      <div className="mb-5 pr-8 sm:mb-7 sm:pr-10">
        <div className="h-[8px] w-full rounded-full bg-[#e5e7eb]">
          <div
            ...
            style={{ width: PROGRESS_WIDTHS[step] }}
          />
        </div>
      </div>

      <DialogTitle className="sr-only">Premium Audit Form</DialogTitle>
      <DialogDescription className="sr-only">
        Multi-step premium audit intake form.
      </DialogDescription>
...
function updateAuditGoals(values: string[], option: string) {
  if (option === "All of the above") {
    return values.includes(option) ? [] : [option];
  }

  const nextValues = updateArrayValue(
    values.filter((value) => value !== "All of the above"),
    option,
  );
...
export default function PremiumAuditModal({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
  ...
  const form = useForm<PremiumAuditFormValues>({
    defaultValues: DEFAULT_VALUES,
    mode: "onTouched",
    resolver: zodResolver(premiumAuditSchema),
  });
  ...
  const auditGoals = watch("auditGoals");
  ...
  const goToNextStep = async () => {
    const isStepValid = await trigger(STEP_FIELDS[step as Step]);
    ...
    if (step < 3) {
      setStep((current: Step) => (current + 1) as Step);
    }
  };
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      // Store contact info for Purchase event on success page
      sessionStorage.setItem('checkout_email', data.email);
      sessionStorage.setItem('checkout_phone', data.phone);

      // Fire Lead — form fully completed
      await trackEvent(
        'Lead',
        { content_name: 'Premium Audit Form', content_category: 'Premium Audit' },
        {
          email: data.email,
          phone: data.phone,
          contentName: 'Premium Audit Form'
        }
      );

      // Fire InitiateCheckout — user is going to pay
      await trackEvent(
        'InitiateCheckout',
        {
          content_name: 'Premium Audit Checkout',
          currency: 'USD',
          value: 149
        },
        {
          ...
          value: 149,
          currency: 'USD',
          contentName: 'Premium Audit Checkout'
        }
      );
      ...
      const response = await createCheckoutSession("Premium Audit", data);
    ...
    >
      <DialogContent className="border-0 bg-transparent p-0 shadow-none">
        ...
        >
          ...
          {step === 2 ? (
            <StepCard step={2} onClose={onClose}>
              <div className="space-y-4 sm:space-y-5">
                ...
                <FieldShell
                  error={errors.payingCustomers?.message}
                  label="Do you have any paying customers?"
                  required
                >
                  <div className="relative">
                    <select
                      className={clsx(
                        INPUT_CLASS,
                        "appearance-none pr-11 text-[#101828]",
                        !watch("payingCustomers") && "text-[#98a2b3]",
                      )}
                      {...register("payingCustomers")}
                    >
                      <option value="">Select Project Stage</option>
                      {PAYING_CUSTOMER_OPTIONS.map((option) => (
                        <option key={option} value={option}>
                          {option}
                        </option>
                      ))}
                  ...
                  <div className="relative">
                    <select
                      className={clsx(
                        INPUT_CLASS,
                        "appearance-none pr-11 text-[#101828]",
                        !watch("revenueModel") && "text-[#98a2b3]",
                      )}
                ...
                <FieldShell
                  error={errors.auditGoals?.message}
                  label="What do you most want to understand from this audit?"
                  required
                >
                  <MultiSelect 
                    options={AUDIT_GOALS}
                    selectedValues={auditGoals}
                    onChange={(newValues) => setValue("auditGoals", newValues, { shouldDirty: true, shouldValidate: true })}
                    placeholder="Select all that apply..."
                  />
                </FieldShell>

                <FieldShell label="Anything else you want us to know? (Optional)">
                  <textarea
                    className={clsx(
                      INPUT_CLASS,
                      "min-h-[70] resize-none py-[10px] placeholder:text-[#98a2b3]",
                    )}
                    placeholder="Any Context that could help our audit..."
                    rows={3}
          ...
          {step === 3 ? (
            <StepCard step={3} onClose={onClose}>
              <div className="space-y-4 sm:space-y-5">
                ...
                <div className="flex flex-col-reverse justify-between gap-3 pt-2 sm:flex-row sm:gap-4">
                  ...
                  <button
                    className="inline-flex h-[56px] w-full items-center justify-center rounded-[80px] bg-[#1C5AF9] px-[28px] font-display text-[15px] font-extrabold uppercase text-white transition hover:bg-[#2a68ff] disabled:opacity-60 sm:h-[58px] sm:w-auto sm:min-w-[242px] sm:px-[50px] sm:text-[16px]"
                    disabled={isSubmitting}
                    type="submit"
                  >
                    {isSubmitting ? "Generating Checkout..." : "Continue to Payment"}
                  </button>
                </div>
              </div>
            </StepCard>
          ...
          {step === 4 && clientSecret && stripePromise ? (
            <StepCard step={4} onClose={onClose}>
              <div className="h-full min-h-[400px] w-full bg-white pb-6 pt-2">
                <EmbeddedCheckoutProvider
